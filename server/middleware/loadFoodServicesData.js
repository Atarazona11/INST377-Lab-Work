...
export async function loadFoodServiceData(req, res, next) {
  try {
    ...
    const replay = json.filter((item) => Boolean(item.geocoded_column_1)).filter((item) => Boolean(item.name));
    console.log('Results in foodServiceData middleware', json.length); // let's check that something's there before we return it
    req.foodServiceData = replay; // and let's _attach_ the data to our request object here
    next();
  } catch (err) {
